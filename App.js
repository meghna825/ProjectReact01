import React from 'react';
import  ReactDOM  from 'react-dom/client';
import  './style.css';
const userImageUrl = 'data:image/png;base64,'
const companyUrl = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSFo9rJTF1-VqhoC2oaTpS9Zez3zDiuao94cQ&usqp=CAU'
const root = ReactDOM.createRoot(document.getElementById('root'))
const TitleReactElemet = React.createElement('div',{className:"title"},
    [React.createElement('h1',{id:"h1"},'Heading1'),
     React.createElement('h2',{id:"h2"},'Heading2'),
     React.createElement('h3',{id:"h3"},'Heading3')
]
)

//root.render(TitleReactElemet)

// Using JSX

const title = (
    <div className='title'>
        <h1>Heading1 from JSX</h1>
        <h2>Heading2 from JSX</h2>
        <h3>Heading3 from JSX</h3>

    </div>
)
//root.render(title)

const Title = ()=> (
    <div className='title'>
        <h1>Heading1 from functional Component</h1>
        <h2>Heading2 from functional Component</h2>
        <h3>Heading3 from functional Component</h3>
</div>
)
const HeaderComponent =()=>(
    <div className='header'>
        <img id = "companyLogo" src = {companyUrl} alt ="companyLogo"/>
        <input type = "text" id = "searchBar" placeholder='Search Food'/>
        <img id = "userImage" src = {userImageUrl}  alt = 'userImg'/>
    </div>
)
//root.render(<Title/>)
root.render(<HeaderComponent/>)